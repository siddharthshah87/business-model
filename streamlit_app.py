...
def run_financial_sim(
    ...
    for yr in range(1, years + 1):
        ...
        arb_revenue = sites * loads_per_site * kwh_shift * 365 * spread * share

        total_kwh_shifted = sites * loads_per_site * kwh_shift * 365
        order_fee = (total_kwh_shifted / 1000) * order2222_fee

        total_revenue = hw_revenue + saas_revenue + arb_revenue
        ...
        ev_ebitda = reported_ebitda * ebitda_multiple if ebitda_multiple > 0 else 0
        ev_revenue = total_revenue * revenue_multiple if revenue_multiple > 0 else 0

        # choose valuation basis: prefer revenue multiple if provided, else EBITDA
        enterprise_value = ev_revenue if revenue_multiple > 0 else ev_ebitda
...
def run_market_sizing(
    ...
    for i in range(projection_years):
        share = som_curve[i]
        som_units_year.append(df["SAM_units"].sum() * share)
    return df, som_units_year

# ============ Streamlit UI ============
st.set_page_config(page_title="Collar + Smart Sub‑Panel Business Model v2", layout="wide")

sidebar = st.sidebar
sidebar.title("Inputs")

projection_years = sidebar.slider("Projection horizon (years)", 3, 10, 5)

sidebar.subheader("Hardware economics")
...
hw_units_year1 = sidebar.number_input("Units shipped in Year 1", 1000, step=100)
hw_unit_growth = sidebar.slider("Annual unit growth rate", 0.0, 1.0, 0.4)

sidebar.subheader("Development & certification (OpEx)")
dev_cost_total = sidebar.number_input("Total dev + UL cost ($)", 2_000_000, step=100_000)
dev_amort_years = sidebar.slider("Amortization horizon (years)", 1, projection_years, 5)

sidebar.subheader("DER / SaaS")
arr_per_site = sidebar.number_input("ARR per site ($/yr)", 120.0, step=10.0)
loads_per_site = sidebar.number_input("Controllable loads per site", 4, step=1)
kwh_shift = sidebar.number_input("kWh shifted per load per day", 3.0, step=0.5)
spread = sidebar.number_input("Wholesale-retail spread ($/kWh)", 0.05, step=0.01)
share = sidebar.slider("Platform share of savings", 0.0, 1.0, 0.4)

sidebar.subheader("Order 2222 aggregation")
mw_year1 = sidebar.number_input("MW under management, Year 1", 4.0, step=0.5)
mw_growth = sidebar.slider("Annual MW growth rate", 0.0, 1.0, 0.6)
order2222_fee = sidebar.number_input("Market fees ($/MWh)", 1.0, step=0.5)

sidebar.subheader("Financial assumptions")
gross_margin_target = sidebar.slider("Blended gross-margin", 0.0, 0.9, 0.35)
core_ebitda_margin = sidebar.slider("Core EBITDA margin", 0.0, 0.5, 0.15)
ebitda_multiple = sidebar.number_input("EV / EBITDA multiple", 8.0, step=0.5)
revenue_multiple = sidebar.number_input("EV / Revenue multiple", 0.0, 50.0, 0.0)

# Tabs
tab_fin, tab_gtm = st.tabs(["Financial Model", "GTM & Market Sizing"])
...
with tab_fin:
    ...
    st.download_button(
        ...
        csv_fin,
        file_name="financial_projection_v2.csv",
        mime="text/csv",
    )

st.caption("v2 • Revenue‑multiple option added. Set either multiple to zero if you want to ignore that basis.")
